// MathIsFun2.cpp : main project file.
...
public:
	...
	void function()		//pure virtual function def
	{
...
class Calculus : public Math		//inheritance req
{
...
public:
	...
	void normalDerivative()
	{
		try
		{
			...
			if (request > 3 || request < 1 || !cin.good())
			{ 
				cin.clear();
				cin.ignore();
		...
		if (request > 3 || request < 1 /*|| !cin.good()*/)	//for some reason doesn't need the third argument to enter this loop for bad input
		{
			normalDerivative(); //recursive call, allows exception handling to mimick a while loop error check
		}
...
// **************************************************************************
//    *             Template Class Dynque - for the Dynamic Queue              *
// **************************************************************************

template <class T>		//class template req
class Dynque		//Dynamic Queues req
{
private:
	...
	{
		T value;
		QueueNode *next;		//ptr variable req
...
// **************************************************************************
//    *             Dynque::enqueue - inserts value at the rear                *
// **************************************************************************
...
void Dynque<T>::enqueue(T val)
{
	if (isEmpty())
	{
		front = rear = new QueueNode(val);		//dynamic memory allocation req
	}
	else
	{
		rear->next = new QueueNode(val);//  FIX ME - 5 WORDS REMOVED
		rear = rear->next;
	}
	numItems++;
}
...
// **************************************************************************
//    *             Dynque::dequeue - removes value at the front               *
// **************************************************************************
...
void Dynque<T>::dequeue(T &val)
{
	...
	{
		cout << "\tDequeuing " << front->value << " from the queue\n\n";

		val = front->value;
		temp = front;
		front = front->next;
		delete temp;//  FIX ME - 5 WORDS 2 LINES REMOVED
		numItems--;
	}
}
...
// **************************************************************************
//    *             Dynque::clear - remove all the elements                    *
// **************************************************************************
...
void Dynque<T>::clear()
{
	...
	while (!isEmpty())
		dequeue(value);//  FIX ME - 2 WORDS REMOVED
}
...
public:
	...
	// Member functions
	CheckRange(int low, int high)           // Constructor
	{
		lower = low;
		upper = high;
	}
	...
	int getInput()
	{
		...
		do
		{
			try      //exception handling req
			{
				cout << "\n\tThis person grabs ticket number ";
				cout << "(" << lower << " to " << upper << ") " << ": ";

				userValue = getNumber();
				cout << "\tYou entered " << userValue << endl;
			}
			catch (CheckRange::OutOfRange ex)
			{
				cout << "\n\tThe value, " << ex.value << ", is out of range.\n";
				userValue = -1;
			}
		} while (userValue < 0);
		return userValue;
...
CheckRange numValue(0, 999999999);
...
void displayMenu()
{
	...
	cout << "\tPress 4 for (Bonus) DMV Simulation" << endl;
...
void queueMenu()
{
	...
	cout << "\n\tPress 1 to open the door (let someone get in line)" << endl;
	cout << "\tPress 2 to take the next person in line" << endl;
	cout << "\tPress 3 to check if anyone is in line" << endl;
	cout << "\tPress 4 to shut down for the day (30 minutes before closing, of course)" << endl;
	cout << "\n\tChoice: ";
	cin >> dmvChoice;

	if (dmvChoice == 1)
	{
		numberValue = numValue.getInput();
		queue.enqueue(numberValue);
	}
